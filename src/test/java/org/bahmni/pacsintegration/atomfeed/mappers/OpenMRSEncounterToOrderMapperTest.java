...
public class OpenMRSEncounterToOrderMapperTest {
    ...
    public void shouldMapOnlyRadiologyOpenMRSOrdersToOrders() throws Exception {
        ...
        when(orderRepository.getByOrderUuid("radiology order uuid")).thenReturn(null);

        Collection<Order> orders = openMRSEncounterToOrderMapper.map(openMRSEncounter, acceptableOrderTypes, orderRepository);

        assertEquals(1, orders.size());
        assertEquals("radiology order uuid", orders.iterator().next().getOrderUuid());
    }
    ...
    public void shouldNotSaveTheOrderWhichIsAlreadySaved() throws Exception {
        ...
        when(orderRepository.getByOrderUuid("radiology order uuid1")).thenReturn(new Order());
        when(orderRepository.getByOrderUuid("radiology order uuid2")).thenReturn(null);

        Collection<Order> orders = openMRSEncounterToOrderMapper.map(openMRSEncounter, acceptableOrderTypes, orderRepository);

        assertEquals(1, orders.size());
        assertEquals("radiology order uuid2", orders.iterator().next().getOrderUuid());
    ...
    public void shouldNotSaveTheOrderIfTheOrderIsVoided() throws Exception {
        ...
        when(orderRepository.getByOrderUuid("radiology order uuid1")).thenReturn(null);
        when(orderRepository.getByOrderUuid("radiology order uuid2")).thenReturn(null);

        Collection<Order> orders = openMRSEncounterToOrderMapper.map(openMRSEncounter, acceptableOrderTypes, orderRepository);

        assertEquals(1, orders.size());
        assertEquals("radiology order uuid1", orders.iterator().next().getOrderUuid());
    }
}